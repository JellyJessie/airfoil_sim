...
export class Shape {
  ...
  constructor(
    ...
    this.camberPct = camberPct;
    this.thicknessPct = thicknessPct;
    this.velocity = velocity;
    this.altitude = altitude;
    this.chord = chord;
    this.span = span;
    this.wingArea = wingArea;
    this.units = options.units ?? 1;
    this.environment = options.environment ?? 1;
  }
  ...
  getReynolds(rho = 1.225, mu = 1.81e-5) {
  ...
  getLiftCoefficient() {
    const a = this.angleDeg * this.getConvDr();
    return 2 * Math.PI * a;
  }

  getLift() {
    const q = 0.5 * 1.225 * this.velocity ** 2;
    return q * this.wingArea * this.getLiftCoefficient();
  }

  getDragCoefficient() {
    const cl = this.getLiftCoefficient();
    return 0.02 + (cl * cl) / (Math.PI * 4);
  }

  getDrag() {
    const q = 0.5 * 1.225 * this.velocity ** 2;
    return q * this.wingArea * this.getDragCoefficient();
  }
  ...
  getVconv() {
    ...
    if (units == "english ▼") vconv = 0.6818;
    else if (units == "metric ▼") vconv = 1.097;
  ...
  getLconv() {
    ...
    if (units == "english ▼") lconv = 1.0;
    else if (units == "metric ▼") lconv = 0.3048;
  ...
  getFconv() {
    ...
    if (units == "english ▼") fconv = 1.0;
    else if (units == "metric ▼") fconv = 4.448;
  ...
  getPconv() {
    ...
    if (units == "english ▼") pconv = 14.7;
    else if (units == "metric ▼") pconv = 101.3;
  ...
  /**
   * This function returns the depht of the shape in the mercury
   */
  getHiteConstMercury() {
    var altitude = this.getAltitude();
    var lconv = this.getLconv();
  ...
  /**
   * This function returns the mercury temperature in Kelvins
   */
  getMercuryTemp() {
    var temp = 520;
    return temp;
  ...
  /**
   * This function returns the density of mercury in slug/ft3
   */
  getRhoMercury() {
    var rho = 1.94;
    return rho;
  ...
  /**
   * This function returns the mercury pressure
   */
  getPressureMercury() {
    var g0;
    if (getUnits() == 1) g0 = 32.2;
    else if (getUnits() == 2) g0 = 9.81;
    var hite = this.getHiteConstMercury();
    var rho = this.getRhoMercury();
    var ps0 = 2116 - rho * g0 * hite;
    return ps0;
  }

  /**
   * This function returns the dynamic pressure of mercury
   */
  getQ0Mercury() {
  ...
  /**
   * This function returns the viscosity of mercury
   */
  getViscosMercury() {
    var ts0 = this.getMercuryTemp();
    var mu0 = this.getMu0Mercury();
    var viscos =
      ((mu0 * 717.408) / (ts0 + 198.72)) * Math.pow(ts0 / 518.688, 1.5);
    return viscos;
  }
   ...
   * environmentSelect = 3, Reynolds number on mercury constant density conditions
...
class Airfoil extends Shape {
  ...
  getLift() {
    ...
    var q0Mercury = this.getQ0Mercury();
    ...
    if (environmentSelect == 1) {
      ...
      lift = (q0Mercury * wingArea * liftCoefficient) / lconv / lconv;
    } else if (environmentSelect == 4) {
  ...
  getDrag() {
    ...
    if (environmentSelect == 1) {
      ...
      drag = (dragCoeff * q0Mercury * area) / lconv / lconv;
      drag = drag * fconv;
    } else if (environmentSelect == 4) {
  ...
  getDragCoefficient() {
    ...
    if (liftAnalisis == 2 || this.getVelocity() == 0) {
      ...
      if (-20.0 <= camd && camd < -10.0) {
        ...
        dragThk10 =
          (camd / 10 + 2) * (dragCam10Thk10 - dragCam20Thk10) + dragCam20Thk10;
        dragThk20 =
          (camd / 10 + 2) * (dragCam10Thk20 - dragCam20Thk20) + dragCam20Thk20;
        ...
        dragThk10 =
          (camd / 10 - 1) * (dragCam20Thk10 - dragCam10Thk10) + dragCam10Thk10;
        dragThk20 =
          (camd / 10 - 1) * (dragCam20Thk20 - dragCam10Thk20) + dragCam10Thk20;
...
class Cylinder extends Shape {
  ...
  getDragCoefficient() {
    ...
    var recyl = [
      ...
      2000, 4000, 5000, 6000, 8000, 10000, 100000, 200000, 400000, 500000,
      600000, 800000, 1000000, 2000000, 4000000, 5000000, 6000000, 8000000,
      1000000000000,
    ];
  ...
  getDrag() {
    ...
    if (environmentSelect == 1) {
      ...
      drag = (dragCoeff * q0Venus * area) / lconv / lconv;
      drag = drag * fconv;
    } else if (this.getVelocity() == 0) drag = 0;
...
class Ball extends Shape {
  ...
  getLiftCoefficient() {
    ...
    if (ar == true) {
      //Correction for aspect ratio
      liftCoefficient =
        liftCoefficient / (1.0 + Math.abs(liftCoefficient) / (pi * 0.2));
    }
  ...
  getLift() {
    ...
    var lift =
      (((((rho * vfsd) / vconv) * gamval * vfsd) / vconv) * span) / lconv; // lift lbs
  ...
  getDragCoefficient() {
    ...
    var resps = [
      ...
      400000, 500000, 600000, 800000, 1000000, 2000000, 4000000, 5000000,
      6000000, 8000000, 1000000000000,
    ];
    var cdsps = [
...
// For consistency with nasa naming:
