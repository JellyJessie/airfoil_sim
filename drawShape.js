...
function draw() {
  ...
  var sldloc = 50;
  var fact = 30.0;
  var spanfac = int(2.0 * fact * aspr * 0.3535);
  var xt = 170;
  var yt = 105;
  var xt1 = xt + spanfac;
  var yt1 = yt - spanfac;
  ...
  if (shapeSelect == 1) {
    ...
    if (getUnits() == 1) rval = cylinder.getRVal() * 1.5;
    else if (getUnits() == 2) rval = cylinder.getRVal() * 1.5;
    ...
    if (getUnits() == 1) rval = ball.getRVal() * 1.5;
    else if (getUnits() == 2) rval = ball.getRVal() * 1.5;
    xcval = ball.getXcVal();
    ycval = ball.getYcVal();
   ...
   */
  if (outputButton == 1 || outputButton == 5) plotGraph(lift, drag);
  else if (outputButton == 4) probe();
  else if (outputButton != 1 || inputButton != 5) Plotly.purge("tester");
   ...
   */
  if (outputButton == 2) geometryButton();
  if (outputButton == 3) dataButton();
  ...
  for (var index = 0; index <= nptc; index++) {
    ...
    rg[0][index] = Math.sqrt(
      xg[0][index] * xg[0][index] + yg[0][index] * yg[0][index]
    );
    thg[0][index] = Math.atan2(yg[0][index], xg[0][index]) / convdr;
    xm[0][index] =
      (rg[0][index] + 1.0 / rg[0][index]) * Math.cos(convdr * thg[0][index]);
    ym[0][index] =
      (rg[0][index] - 1.0 / rg[0][index]) * Math.sin(convdr * thg[0][index]);
    ...
    if (environmentSelect == 1) {
      ...
      /**
       * environmentSelect = 3, mercury constant density
       * ps0 = pressure, mercury constant density
       * q0 = dynamic pressure, mercury constant density
       */
      ps0 = airfoil.getPressureMercury();
    ...
    if (shapeSelect < 4) mapfac = 4.0;
    else if (shapeSelect >= 4) mapfac = 2.0;
    ...
    for (var i = 0; i <= 1; i++) {
      ...
      if (shapeSelect < 4) {
         ...
         */
        if (view == 1) {
          if (getUnits() == 1) polygon(exes[1], whys[1], radius * 20, 20);
          else if (getUnits() == 2)
            polygon(exes[1], whys[1], (radius * 20) / 0.3, 20);
          ...
          fill("white");
          stroke("red");
          if (getUnits() == 1) polygon(exes[2], whys[2], radius * 20, 20);
          else if (getUnits() == 2)
            polygon(exes[2], whys[2], (radius * 20) / 0.3, 20);
          fill("red");
          if (getUnits() == 1)
            polygon(
              exes[2] + (index * span) / 7,
            ...
            polygon(
              exes[2] + (index * span) / 7 / 0.3,
              whys[2] - (index * span) / 7 / 0.3,
              (radius * 20) / 0.3,
              20
            );
    ...
    if (shapeSelect >= 4 && view == 1) {
      exes[0] =
        int(
          fact *
            (0.01 * (xm[0][1] + xm[0][2]) +
              (rval / 2.0) * Math.cos(convdr * (plthg[1] + 180)))
        ) + xt;
      whys[0] =
        int(
          fact *
            (-ym[0][1] + (rval / 2.0) * Math.sin(convdr * (plthg[1] + 180)))
        ) + yt;
      exes[1] =
        int(
          fact *
            (0.01 * (xm[0][1] + xm[0][2]) +
              (rval / 2.0) * Math.cos(convdr * plthg[1]))
        ) + xt;
      whys[1] =
        int(fact * (-ym[0][1] + (rval / 2.0) * Math.sin(convdr * plthg[1]))) +
        yt;
      push();
      stroke("red");
  ...
  if (display == 1) {
    ...
    for (j = 1; j <= nln2 - 1; j = j + 1) {
      ...
      for (i = 1; i <= nptc - 1; i = i + 1) {
        ...
        if (environmentSelect <= 2 || environmentSelect == 4) stroke("yellow");
        else if (environmentSelect == 3) stroke("blue");
        if (velocity == 0) stroke("black");

        line(exes[0], whys[0], exes[1], whys[1]);
      }
    }

    for (j = nln2 + 1; j <= nlnc; ++j) {
      // upper half
      for (i = 1; i < nptc - 1; i++) {
        exes[0] = int(fact * xm[j][i]) + xt;
        whys[0] = int(fact * -ym[j][i]) + yt;
        ...
        if (environmentSelect == 1 || environmentSelect == 4) stroke("cyan");
        else if (environmentSelect == 2) stroke("yellow");
        else if (environmentSelect == 3) stroke("blue");
        if (velocity == 0) stroke("black");
    ...
    for (j = 1; j <= nln2; ++j) {
      ...
      for (i = 1; i < 27; i = i + 2) {
        ...
        if ((i % 5 == 0 || i % 9 == 0) && velocity != 0) stroke("white");
        else if (velocity == 0) stroke("black");
        else stroke("cyan");

        if (environmentSelect == 3) stroke("blue");
...
function getLyg(fxg, psv, alfval, rval, gamval) {
  ...
  if (Math.abs(psv) < 0.001 && alfval < 0.0) {
    ynew = rval;
  }
  if (Math.abs(psv) < 0.001 && alfval >= 0.0) {
    ynew = -rval;
